...
const SystemSize = (() => {
  ...
  // Functions
  function init() {
    const li = document.querySelector('li.size')
    if (!li) { return }
    eventBinding()
    get_sizes()
  }
  ...
  function add_size(e) {
    ...
    /* const html = `<li class="size">
                      <h3>Module #
                        <span class="size__number">${++state.current_module}</span>
                        <span class="delete">Delete</span>
                      </h3>
                      <input type="text" class="size__input" name="size__quantity" placeholder="Size of the module" data-module="${state.current_module}">
                     </li>`*/

    // Li
    const mainNode = document.createElement('li')
    mainNode.className = 'size'
    ...
    _input.setAttribute('placeholder', 'Size of the module')
